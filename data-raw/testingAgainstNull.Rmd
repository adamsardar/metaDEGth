...
// [[Rcpp::export]]
double pHyperExponentialTail(double testStat, NumericVector generatorProbabilities, NumericVector rates) {
  
  boost::math::hyperexponential hypexp(generatorProbabilities, rates);
  
  return boost::math::cdf(boost::math::complement(hypexp, testStat));
}
...
for(i in 1:1E4){
  ...
  random_draws <- map(1:1E4, ~{rbetaUniformFunc(randomSetSize, fittedBetaShape, uniformProportion)})
...
ggplot(data = data.frame(x = c(0, 100)), aes(x)) +
  ...
  stat_function(fun = dphtype, args = list(prob = c(c(uniformProportion, 1-uniformProportion), rep(0,2*(randomSetSize-1))),
